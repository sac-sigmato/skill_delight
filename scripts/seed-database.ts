...
async function seedDatabase() {
  try {
    ...
    console.log('\n📊 Summary:');
    console.log(`👤 Users: ${savedStudents.length + 1} (${savedStudents.length} students + 1 admin)`);
    console.log(`📚 Courses: ${savedCourses.length}`);
    console.log(`📅 Course Slots: ${savedSlots.length}`);
    console.log(`📝 Enrollments: ${savedEnrollments.length}`);
    console.log(`💳 Transactions: ${savedTransactions.length}`);
    console.log(`🔔 Notifications: ${savedNotifications.length}`);
    console.log('\n🔑 Admin Login:');
    console.log('Email: admin@skilldelight.com');
